...
function SelectTrigger({
  ...
  return (
    <SelectPrimitive.Trigger
      ...
      className={cn(
        controlClasses(
          "data-placeholder:text-control-foreground-muted flex w-fit items-center justify-between gap-1.5 py-2 pr-2 pl-2.5 whitespace-nowrap data-[size=sm]:h-8 *:data-[slot=select-value]:line-clamp-1 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-1.5 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4"
        ),
        className
      )}
      {...props}
    >
      ...
      <SelectPrimitive.Icon
        render={
          <ChevronDownIcon className="text-control-foreground-muted pointer-events-none size-4" />
        }
      />
    </SelectPrimitive.Trigger>
  );
